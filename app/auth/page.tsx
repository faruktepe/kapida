...
export default function AuthPage() {
  ...
  const handleKayit = async () => {
    ...
    const { error: err } = await supabase.auth.signUp({
      ...
      password: sifre,
      options: {
        data: { full_name: ad, phone: telefon },
      },
    });
  ...
  return (
    <main className="min-h-screen flex items-center justify-center px-6 py-16" style={{ background: BG }}>
      <div className="w-full max-w-md">
        ...
        <div className="text-center mb-10">
          <Link href="/">
            <img src="/temizgelsin-logo.png?v=1" alt="Temiz Gelsin"
              style={{ height: "48px", width: "auto", margin: "0 auto 20px" }} />
          </Link>
          <h1 className="text-2xl font-black tracking-tight" style={{ color: DRK }}>
            {mod === "giris" && "Giriş Yap"}
            {mod === "kayit" && "Hesap Oluştur"}
            {mod === "sifre" && "Şifremi Unuttum"}
          </h1>
          <p className="text-sm mt-1" style={{ color: `rgba(45,26,46,0.45)` }}>
            {mod === "giris" && "Siparişlerini takip etmek için giriş yap."}
            {mod === "kayit" && "Saniyeler içinde hesabını oluştur."}
            {mod === "sifre" && "E-postana sıfırlama bağlantısı göndereceğiz."}
          </p>
        </div>

        {/* Kart */}
        <div className="rounded-3xl p-8 shadow-sm" style={{ background: "#fff", border: `1.5px solid ${STN}` }}>

          {/* Tab: Giriş / Kayıt */}
          {mod !== "sifre" && (
            <div className="flex rounded-xl overflow-hidden mb-6 p-1" style={{ background: `rgba(212,197,176,0.2)` }}>
              {(["giris", "kayit"] as const).map((m) => (
                <button key={m} onClick={() => { setMod(m); setError(""); setSuccess(""); }}
                  className="flex-1 py-2.5 text-sm font-bold rounded-lg transition-all"
                  style={mod === m
                    ? { background: PRI, color: MUV, boxShadow: "0 2px 8px rgba(91,45,110,0.2)" }
        ...
        {/* Alt not */}
        <p className="text-center text-xs mt-6" style={{ color: `rgba(45,26,46,0.3)` }}>
          Giriş yaparak{" "}
