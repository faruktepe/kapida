...
const DETAYLAR = ["Profesyonel renk analizi","Çizik ve bozuk alan tamiri","El boyama tekniği","Renk sabitleme kaplaması","Son rötuş ve parlatma"];
const ADIMLAR  = ["Ayakkabı teslim alınır ve analiz edilir","Yüzey temizlenir ve hazırlanır","Profesyonel boya uygulanır","Kurutma ve sabitleme işlemi","Koruyucu kaplama ve teslim"];
...
export default function Page() {
  ...
  useEffect(() => {
    supabase.from("gallery").select("id,title,before_url,after_url").eq("active",true).eq("category","deri").limit(3).then(({ data }) => setGaleri(data??[]));
  }, []);
    ...
    <main className="min-h-screen" style={{background: BG, color: DRK}}>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 pt-28 pb-24">
        <div className="pb-12 mb-12" style={{borderBottom:`1px solid ${STN}`}}>
          <p className="text-[10px] uppercase tracking-[0.4em] mb-3" style={{color:`rgba(45,26,46,0.25)`}}>02 · Hizmet</p>
          <h1 className="text-4xl md:text-7xl font-black tracking-tight leading-none mb-6" style={{color: DRK}}>Boya & Restorasyon</h1>
          <p className="text-base max-w-lg leading-relaxed mb-8" style={{color:`rgba(45,26,46,0.5)`}}>Rengi solmuş ya da yıpranmış ayakkabılar için profesyonel renk yenileme ve restorasyon.</p>
          <div className="flex flex-wrap gap-4 items-center">
            <Link href="/siparis" className="inline-flex items-center gap-3 px-8 py-4 text-sm font-bold rounded-full hover:opacity-90 transition-all" style={{background: PRI, color: MUV}}>
              Hemen Sipariş Ver →
            </Link>
            <div className="flex gap-6">
              <div>
                <p className="text-2xl font-black" style={{color: ACCENT}}>₺1.200+</p>
                <p className="text-[10px] mt-0.5 uppercase tracking-wider" style={{color:`rgba(45,26,46,0.3)`}}>Başlangıç</p>
              </div>
              <div>
                <p className="text-2xl font-black" style={{color: PRI}}>2-3 gün</p>
        ...
        {galeri.length > 0 && (
          <div className="mb-16">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {galeri.map(item => (
                <div key={item.id} className="rounded-2xl overflow-hidden cursor-pointer border hover:shadow-md transition-all"
                  style={{borderColor: STN}} onMouseEnter={() => setHover(item.id)} onMouseLeave={() => setHover(null)}>
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <span className="absolute top-3 left-3 text-[9px] px-2 py-1 rounded-full font-bold"
                      style={{background: hover===item.id ? PRI : `rgba(45,26,46,0.6)`, color: MUV}}>
