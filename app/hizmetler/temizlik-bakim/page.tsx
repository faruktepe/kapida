  ...
  return (
    <nav className="fixed top-0 left-0 right-0 z-50 flex items-center justify-between px-6 md:px-12 py-4 backdrop-blur-md border-b" style={{background:"rgba(245,240,232,0.95)", borderColor:"rgba(212,197,176,0.4)"}}>
      ...
      <div className="flex items-center gap-2">
        <Link href="/hizmetler" className="hidden md:block px-3 py-1.5 text-xs transition-colors" style={{color:"rgba(0,0,0,0.4)"}}>← Hizmetler</Link>
    ...
    <main className="min-h-screen text-black" style={{background:"#F5F0E8"}}>
      ...
      <div className="max-w-5xl mx-auto px-6 md:px-12 pt-28 pb-24">
        ...
        <div className="pb-12 mb-12" style={{borderBottom:"1px solid rgba(212,197,176,0.5)"}}>
          <p className="text-[10px] uppercase tracking-[0.4em] mb-3" style={{color:"rgba(0,0,0,0.25)"}}>01 · Hizmet</p>
          ...
          <p className="text-base max-w-lg leading-relaxed mb-8" style={{color:"rgba(0,0,0,0.5)"}}>Ayakkabınızı fabrika çıkışı gibi yeniliyoruz. Buhar temizlik, derin cila ve UV sterilizasyon ile tertemiz teslim.</p>
          <div className="flex flex-wrap gap-4 items-center">
            ...
            <div className="flex gap-6">
              <div><p className="text-2xl font-black" style={{color:MOR}}>₺800+</p><p className="text-[10px] mt-0.5 uppercase tracking-wider" style={{color:"rgba(0,0,0,0.3)"}}>Başlangıç</p></div>
              <div><p className="text-2xl font-black" style={{color:"#6B2737"}}>1–2 gün</p><p className="text-[10px] mt-0.5 uppercase tracking-wider" style={{color:"rgba(0,0,0,0.3)"}}>Süre</p></div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-16 mb-16">
          <div>
            <p className="text-[10px] uppercase tracking-[0.4em] mb-6" style={{color:"rgba(0,0,0,0.25)"}}>Kapsam</p>
          ...
          <div>
            <p className="text-[10px] uppercase tracking-[0.4em] mb-6" style={{color:"rgba(0,0,0,0.25)"}}>Süreç</p>
            <div className="space-y-4">
              {ADIMLAR.map((a, i) => (
                <div key={i} className="flex gap-4">
                  <span className="text-[10px] font-mono mt-0.5 shrink-0" style={{color:"rgba(0,0,0,0.2)"}}>{String(i + 1).padStart(2, "0")}</span>
        ...
        {galeri.length > 0 && (
          <div className="mb-16">
            <p className="text-[10px] uppercase tracking-[0.4em] mb-6" style={{color:"rgba(0,0,0,0.25)"}}>Öncesi / Sonrası</p>
              ...
              {galeri.map(item => (
                <div key={item.id} className="rounded-2xl overflow-hidden cursor-pointer border" style={{borderColor:"rgba(212,197,176,0.5)"}} onMouseEnter={() => setHover(item.id)} onMouseLeave={() => setHover(null)}>
                  <div className="relative h-48 overflow-hidden">
                    ...
                    <span className="absolute top-3 left-3 text-[9px] px-2 py-1 rounded-full font-medium text-white" style={{background: hover === item.id ? MOR : "rgba(0,0,0,0.5)"}}>{hover === item.id ? "Sonra" : "Önce"}</span>
                  </div>
        ...
        <div className="pt-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-6" style={{borderTop:"1px solid rgba(212,197,176,0.5)"}}>
          <div>
            ...
            <p className="text-sm mt-1" style={{color:"rgba(0,0,0,0.4)"}}>Kapıdan kapıya — 24 saat içinde teslim alıyoruz.</p>
          </div>
