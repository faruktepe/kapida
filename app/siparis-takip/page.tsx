...
const DURUMLAR: Record<string, {label:string; bg:string; color:string; desc:string}> = {
  "Teklif Bekleniyor": {label:"Teklif Bekleniyor", bg:`rgba(191,165,184,0.15)`, color: MUV,  desc:"Fiyat teklifinizi hazırlıyoruz."},
  "Onaylandı":         {label:"Onaylandı",         bg:`rgba(91,45,110,0.08)`,   color: PRI,  desc:"Siparişiniz onaylandı, kurye yolda."},
  "Kuryede":           {label:"Kuryede",            bg:`rgba(91,45,110,0.1)`,    color: PRI,  desc:"Kurye ayakkabınızı teslim almaya geliyor."},
  "İşlemde":           {label:"İşlemde",            bg:`rgba(191,165,184,0.15)`, color: MUV,  desc:"Ayakkabınız bakımda, özenle çalışıyoruz."},
  "Tamamlandı":        {label:"Tamamlandı",         bg:`rgba(91,45,110,0.08)`,   color: PRI,  desc:"Bakım tamamlandı, kurye yola çıkıyor."},
  "Teslim Edildi":     {label:"Teslim Edildi",      bg: DRK,                     color: MUV,  desc:"Ayakkabınız kapınıza teslim edildi."},
};
...
export default function SiparisTakipPage() {
  ...
  const durum     = order ? (DURUMLAR[order.status as string] ?? {label: order.status as string, bg:`rgba(212,197,176,0.2)`, color: DRK, desc:""}) : null;
      ...
      <div className="max-w-2xl mx-auto px-6 md:px-12 py-16">
        ...
        <div className="flex gap-3 mb-8">
          <input value={no} onChange={e => setNo(e.target.value)}
            ...
            style={{border:`2px solid ${STN}`, background:"#fff", color: DRK}}
            placeholder="KPD-XXXXXXXX" />
          <button onClick={sorgula} disabled={loading}
            className="px-8 py-3.5 text-sm font-bold rounded-xl hover:opacity-90 transition-all disabled:opacity-50"
        ...
        {error && (
          <div className="p-4 rounded-xl text-sm mb-6 font-medium"
            style={{background:`rgba(191,165,184,0.15)`, border:`1px solid ${MUV}`, color: DRK}}>
            {error}
          </div>
        )}
          ...
          <div className="space-y-4">
            ...
            <div className="rounded-2xl p-6 border-2" style={{background: durum.bg, borderColor:`${durum.color}40`}}>
              <div className="flex items-center justify-between mb-2">
                <span className="font-mono text-sm" style={{color:`rgba(45,26,46,0.4)`}}>{order.order_number as string}</span>
                <span className="text-xs font-bold px-3 py-1 rounded-full" style={{background: durum.color, color: durum.bg === DRK ? MUV : "#fff"}}>
                  {durum.label}
                </span>
              </div>
            ...
            {/* İlerleme */}
            <div className="rounded-2xl p-6" style={{background:"#fff", border:`1px solid ${STN}`}}>
              <p className="text-[10px] uppercase tracking-widest mb-4" style={{color:`rgba(45,26,46,0.3)`}}>İlerleme</p>
              <div className="flex items-center gap-1">
                {ADIMLAR.map((adim, i) => (
                  <div key={adim} className="flex items-center flex-1">
                    <div className="flex flex-col items-center flex-1">
                      <div className="w-3 h-3 rounded-full transition-all"
                        style={{background: i <= aktifAdim ? PRI : STN}} />
                      <p className="text-[8px] text-center mt-1 hidden md:block leading-tight max-w-[60px]"
                        style={{color:`rgba(45,26,46,0.3)`}}>{adim}</p>
                    </div>
                    {i < ADIMLAR.length - 1 && (
            ...
            {/* Ayakkabı bilgisi */}
            <div className="rounded-2xl p-6" style={{background:"#fff", border:`1px solid ${STN}`}}>
              <p className="text-[10px] uppercase tracking-widest mb-4" style={{color:`rgba(45,26,46,0.3)`}}>Ayakkabı</p>
              <div className="grid grid-cols-2 gap-3 text-sm mb-4">
                <div><p className="text-xs mb-1" style={{color:`rgba(45,26,46,0.4)`}}>Marka</p><p className="font-semibold" style={{color: DRK}}>{order.brand as string}</p></div>
                {order.model && <div><p className="text-xs mb-1" style={{color:`rgba(45,26,46,0.4)`}}>Model</p><p className="font-semibold" style={{color: DRK}}>{order.model as string}</p></div>}
                <div><p className="text-xs mb-1" style={{color:`rgba(45,26,46,0.4)`}}>Renk</p><p className="font-semibold" style={{color: DRK}}>{order.color as string}</p></div>
                <div><p className="text-xs mb-1" style={{color:`rgba(45,26,46,0.4)`}}>Tür</p><p className="font-semibold" style={{color: DRK}}>{order.shoe_type as string}</p></div>
              </div>
              <p className="text-[10px] uppercase tracking-widest mb-2" style={{color:`rgba(45,26,46,0.3)`}}>Hizmetler</p>
              <div className="flex flex-wrap gap-2">
                {hizmetler.map((h: string) => (
                  <span key={h} className="text-xs px-3 py-1 rounded-full"
                    style={{border:`1px solid ${STN}`, color:`rgba(45,26,46,0.6)`}}>{h}</span>
                ))}
              </div>
            </div>
            ...
            <div className="rounded-2xl p-6 flex justify-between items-center"
              ...
              <p className="text-sm" style={{color:`rgba(45,26,46,0.5)`}}>Tahmini Fiyat</p>
              <p className="text-xl font-black" style={{color: PRI}}>{order.price as string}</p>
            </div>

            {musteriInfo && (
              <div className="rounded-2xl p-6" style={{background:"#fff", border:`1px solid ${STN}`}}>
                <p className="text-[10px] uppercase tracking-widest mb-4" style={{color:`rgba(45,26,46,0.3)`}}>Teslimat</p>
                <div className="grid grid-cols-2 gap-3 text-sm">
                  <div><p className="text-xs mb-1" style={{color:`rgba(45,26,46,0.4)`}}>Ad</p><p className="font-semibold" style={{color: DRK}}>{musteriInfo.ad}</p></div>
                  <div><p className="text-xs mb-1" style={{color:`rgba(45,26,46,0.4)`}}>İlçe</p><p className="font-semibold" style={{color: DRK}}>{musteriInfo.ilce}</p></div>
                </div>
              </div>
            )}
