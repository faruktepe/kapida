...
const HIZMETLER = [
  { id: "bakim",      label: "Temizlik & Bakım",    desc: "Buhar temizlik, cila, bağcık değişimi",    fiyat: 800  },
  { id: "boya",       label: "Boya & Restorasyon",  desc: "Profesyonel boya ve renk yenileme",        fiyat: 1200 },
  { id: "taban",      label: "Taban Değişimi",       desc: "Dış taban komple değişimi",               fiyat: 600  },
  { id: "dikiş",      label: "Dikiş Tamiri",         desc: "Yırtık ve sökük dikişlerin tamiri",       fiyat: 400  },
  { id: "ic_taban",   label: "İç Taban Değişimi",    desc: "İç taban komple değişimi",                fiyat: 400  },
  { id: "coraplık",   label: "Çoraplık Tamiri",      desc: "Arka iç kısım onarımı",                   fiyat: 350  },
  { id: "fort",       label: "Fort Tamiri",           desc: "Arka dik kısım tamiri",                   fiyat: 300  },
  { id: "yapıştırma", label: "Yapıştırma",            desc: "Taban yapıştırma ve sökük onarımı",      fiyat: 250  },
];
  ...
  if (step === 4) return (
    <main className="min-h-screen flex flex-col items-center justify-center px-6 py-20" style={{background: BG}}>
      <div className="max-w-md w-full text-center">
        <div className="w-20 h-20 rounded-full flex items-center justify-center text-3xl mx-auto mb-8" style={{background: PRI, color: MUV}}>✓</div>
        <h1 className="text-4xl font-black mb-3 tracking-tight" style={{color: DRK}}>Sipariş Alındı!</h1>
        <p className="text-sm mb-2" style={{color:`rgba(45,26,46,0.45)`}}>Sipariş numaranız</p>
        <div className="flex items-center gap-2 mb-3 justify-center">
          <p className="font-mono font-bold text-xl px-6 py-3 rounded-2xl" style={{background:"#fff", border:`1px solid ${STN}`, color: DRK}}>{orderNumber}</p>
        ...
        <p className="text-sm mb-10 mt-2" style={{color:`rgba(45,26,46,0.5)`}}>
    ...
    <main className="min-h-screen" style={{background: BG}}>
      ...
      <header className="sticky top-0 z-50 flex items-center justify-between px-6 md:px-10 h-16 border-b backdrop-blur-md"
        ...
        <Link href="/" className="flex items-center">
          <img src="/logo-clean.png?v=4" alt="Kapıda" style={{height:"44px", width:"auto"}} />
        </Link>
            ...
            <div key={i} className="flex items-center gap-1.5">
              <div className="w-7 h-7 rounded-full flex items-center justify-center text-[11px] font-bold transition-all"
                style={step > i+1
                  ...
                  : step === i+1
                    ? {background: PRI, color: MUV}
                    : {background:`rgba(212,197,176,0.4)`, color:`rgba(45,26,46,0.4)`}}>
                {step > i+1 ? "✓" : i+1}
              </div>
              <span className="text-[11px] hidden md:block font-medium" style={{color: step === i+1 ? DRK : `rgba(45,26,46,0.3)`}}>{l}</span>
              {i < 2 && <div className="w-6 h-px mx-1 hidden md:block" style={{background: STN}} />}
            </div>
          ))}
        </div>
      </header>
      ...
      <div className="max-w-lg mx-auto px-6 py-12">
        ...
        {step === 1 && (
          <div>
            <p className="text-[10px] uppercase tracking-[0.35em] mb-2" style={{color:`rgba(45,26,46,0.3)`}}>Adım 1 / 3</p>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight mb-8" style={{color: DRK}}>
              Ayakkabı <span style={{color: PRI}}>bilgileri</span>
            </h1>
            <div className="space-y-5">
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>Marka *</label>
              ...
              {form.marka && markaModeller.length > 0 && (
                <div>
                  <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>Model</label>
              ...
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>Renk *</label>
                <div className="flex flex-wrap gap-2">
                  {RENKLER.map(r => (
                    <button key={r} onClick={() => setForm(f=>({...f,renk:r}))}
                      ...
                      style={form.renk === r ? btnSel : btnDef}>
                      {r}
                    </button>
                  ))}
                </div>
              ...
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>Ayakkabı türü *</label>
                <div className="flex flex-wrap gap-2">
                  {TURLER.map(t => (
        ...
        {step === 2 && (
          <div>
            <p className="text-[10px] uppercase tracking-[0.35em] mb-2" style={{color:`rgba(45,26,46,0.3)`}}>Adım 2 / 3</p>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight mb-2" style={{color: DRK}}>
              Hizmet <span style={{color: PRI}}>seçin</span>
            </h1>
            <p className="text-sm mb-6" style={{color:`rgba(45,26,46,0.45)`}}>Birden fazla seçebilirsiniz. 3+ seçimde %20 indirim!</p>

            {kampanya && (
              <div className="flex items-center gap-2 text-xs px-4 py-2.5 rounded-full mb-6 w-fit font-bold"
              ...
              {HIZMETLER.map(h => {
                ...
                return (
                  <button key={h.id} onClick={() => toggleHizmet(h.id)}
                    ...
                    style={sel ? {borderColor: PRI, background:`rgba(91,45,110,0.04)`} : {borderColor: STN, background:"#fff"}}>
                    <div className="w-6 h-6 rounded-full border-2 flex items-center justify-center shrink-0 transition-all text-[11px] font-bold"
                      style={sel ? {borderColor: PRI, background: PRI, color: MUV} : {borderColor: STN}}>
                      {sel ? "✓" : ""}
                    </div>
                    <div className="flex-1">
                      <div className="flex justify-between items-center">
                        <span className="font-semibold text-sm" style={{color: DRK}}>{h.label}</span>
                        <span className="text-sm font-bold" style={{color: sel ? PRI : `rgba(45,26,46,0.35)`}}>₺{h.fiyat}+</span>
                      </div>
            ...
            {secilenHizmetler.length > 0 && (
              <div className="p-5 rounded-2xl mb-6 border-2" style={{borderColor: PRI, background:`rgba(91,45,110,0.04)`}}>
                <div className="flex justify-between items-center">
                  <span className="text-sm" style={{color:`rgba(45,26,46,0.5)`}}>Tahmini fiyat</span>
                  <span className="text-xl font-black" style={{color: PRI}}>₺{indirimliMin} — ₺{indirimliMax}</span>
                </div>
                {kampanya && <p className="text-xs mt-1 font-semibold" style={{color: PRI}}>%20 indirim uygulandı 🎉</p>}
              </div>
            )}
            ...
            <div className="flex gap-3">
              <button onClick={() => setStep(1)}
                className="flex-1 border-2 py-4 text-sm font-semibold rounded-full transition-all"
        ...
        {step === 3 && (
          <div>
            <p className="text-[10px] uppercase tracking-[0.35em] mb-2" style={{color:`rgba(45,26,46,0.3)`}}>Adım 3 / 3</p>
            <h1 className="text-3xl md:text-4xl font-black tracking-tight mb-8" style={{color: DRK}}>
              İletişim <span style={{color: PRI}}>bilgileri</span>
            </h1>
            <div className="space-y-4">
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>Ad Soyad *</label>
              ...
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>Telefon *</label>
              ...
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>İlçe *</label>
                <div className="flex flex-wrap gap-2">
                  {ILCELER.map(ilce => (
              ...
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>Adres *</label>
              ...
              {/* REFERANS KODU */}
              <div>
                <label className="text-[11px] uppercase tracking-widest block mb-2" style={{color:`rgba(45,26,46,0.4)`}}>
                  Referans Kodu <span className="normal-case" style={{color:`rgba(45,26,46,0.25)`}}>(isteğe bağlı)</span>
                </label>
                <div className="flex gap-2">
                  <div className="relative flex-1">
                    <input value={form.referralCode}
                      onChange={e => { const val = e.target.value.toUpperCase(); setForm(f=>({...f,referralCode:val})); setReferralValid(null); setReferralDiscount(0); setReferralMsg(""); }}
                    ...
                    {referralValid === true  && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-bold" style={{color: PRI}}>✓</span>}
                    {referralValid === false && <span className="absolute right-3 top-1/2 -translate-y-1/2 text-sm font-bold" style={{color:`rgba(107,39,55,0.7)`}}>✗</span>}
                  </div>
                ...
                {referralMsg
                  ...
                  : <p className="text-xs mt-1.5" style={{color:`rgba(45,26,46,0.3)`}}>Arkadaşından aldığın referans koduyla indirim kazan</p>
                }
              </div>
            </div>

            {/* ÖZET */}
            <div className="rounded-2xl p-5 my-6" style={{border:`2px solid ${STN}`}}>
              <p className="text-[10px] uppercase tracking-widest mb-3" style={{color:`rgba(45,26,46,0.3)`}}>Sipariş özeti</p>
              {secilenHizmetler.map(h => (
                <div key={h.id} className="flex justify-between text-sm py-1.5" style={{borderBottom:`1px solid rgba(212,197,176,0.5)`}}>
                  <span style={{color: DRK}}>{h.label}</span>
                  <span className="font-medium" style={{color:`rgba(45,26,46,0.4)`}}>₺{h.fiyat}+</span>
                </div>
              ))}
              {kampanya && (
                <div className="flex justify-between text-xs py-1.5 font-semibold" style={{color: PRI}}>
                  <span>✦ 3+ hizmet indirimi</span><span>-%20</span>
                </div>
              )}
              {referralValid && referralDiscount > 0 && (
                <div className="flex justify-between text-xs py-1.5 font-semibold" style={{color: MUV}}>
            ...
            <div className="space-y-3 mb-8">
              {[
                ...
                <button key={opt.val} onClick={() => setForm(f=>({...f,tercih:opt.val as "arasin"|"onayla"}))}
                  ...
                  style={form.tercih === opt.val ? {borderColor: PRI, background:`rgba(91,45,110,0.04)`} : {borderColor: STN, background:"#fff"}}>
                  ...
                  <div>
                    <p className="font-semibold text-sm" style={{color: DRK}}>{opt.title}</p>
